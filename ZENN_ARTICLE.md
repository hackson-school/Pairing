# 【Next.js × 3Dページフリップ】「お菓子と飲み物の手帖」を作った話 〜 スマホ激重問題・タップ貫通・Canvas画像保存の泥臭い戦い 〜
...
アプリ全体のデータフローとインタラクションの構造は以下のようになっています：

```mermaid
graph TD
    A[初期画面: 3D閉じた洋書] -->|クリック| B[3D表紙オープン & パラパラめくり]
    B -->|ズームイン・トランジション| C[メイン画面: 手帖アプリ]
    
    subgraph 手帖インタラクション
        C --> D[Page 1-2: イントロ & ペアリング三原則]
        D -->|3Dフリップ| E[Page 3: お菓子選択 写真 / 定番8種 / 手入力]
        E -->|3Dフリップ| F[Page 4: 合わせたい一杯 & 志向指定]
        F -->|Gemini API 科学鑑定| G[Page 5-6: 診断結果 & サービング処方]
    end
    
    subgraph シェア & 保存
        G --> H[HTML5 Canvas 2D レンダラー]
        H --> I[高画質手帖カードPNG]
        I --> J[長押し保存 / ダウンロード / Web Share API]
    end
```
...
```
Next.js × 3Dページフリップで「お菓子と飲み物の手帖」を作りました！📖✨

モバイル実機で直面した
・アドレスバー伸縮による無限再描画ループ
・ボタンタップの貫通・ページ飛ばし
・Safariでの画像保存失敗
など泥臭いフロントエンド改善の記録をZennにまとめました！👇

#Zenn #Nextjs #React #Webデザイン
[記事URL]
```
