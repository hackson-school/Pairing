...
export default function BookIntroTransition({ children }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-[#F0EEE9] flex items-center justify-center overflow-hidden">
      {/* ヘッダーテキスト */}
      <AnimatePresence>
        {isClosed && (
          <motion.div
      ...
      <motion.div
        animate={{
          x: isClosed ? PAGE_W / 2 : 0,
          scale: isZooming ? 22 : 1,
      ...
      >
        ...
        >
          {/* ── 左ページ（開いたときに現れる） ── */}
          <motion.div
            className="absolute left-0 top-0 bg-[#FAF8F4] flex flex-col items-center justify-center"
            style={{
              width: PAGE_W,
              height: PAGE_H,
          ...
          {/* ── 右ページ（ベース・常に表示） ── */}
          <div
            className="absolute top-0 bg-[#F5F2EB]"
            style={{
              left: PAGE_W,
              width: PAGE_W,
              height: PAGE_H,
          ...
          >
            {/* ［表紙の表面］（閉じたときに見える面） */}
            <div
              className="absolute inset-0 rounded-r-lg text-[#FAF8F4] overflow-hidden"
              style={{
            ...
            >
              <div className="absolute inset-4 border border-[#8C532B]/40 rounded-lg pointer-events-none" />
            ...
            {/* ［表紙の裏面（見返し紙）］（開いたときに左側に見える面） */}
            <div
              className="absolute inset-0 rounded-l-lg overflow-hidden flex flex-col items-center justify-center p-8 text-center select-none"
              style={{
      ...
      <AnimatePresence>
        {isClosed && (
          <motion.p
            key="hint"
            className="absolute bottom-10 font-sans text-xs text-charcoal-400 tracking-wider"
            initial={{ opacity: 0 }}
